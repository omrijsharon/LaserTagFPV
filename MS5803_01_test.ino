...
*/
  // Calculate altitude
  h = calc_altitude(sensor.pressure(), p0, sensor.temperature()) * transition_rate + last_h * (1-transition_rate);
  last_h = h;
...
float calc_altitude(float p, float p0, float temperature) {
  // this function calculate the altitude as function of pressure, temperature and initial pressure
  return (1-(pow(p/p0, 1/35.009))) * temperature / 0.00976;
}